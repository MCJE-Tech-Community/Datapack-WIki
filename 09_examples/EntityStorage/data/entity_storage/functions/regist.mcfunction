...
## idを付与
    # 未使用idがない時
    execute unless data storage entity_storage:core unused_id[-1] store result score @s estS.id run scoreboard players add estH.new estS.id 1
    execute unless data storage entity_storage:core unused_id[-1] run data modify storage entity_storage: data append value {}
    # 未使用idがあるとき
...
## UUIDまたは名前を取得
    # データ
    execute store result storage entity_storage:core macro.id int 1 run scoreboard players get @s estS.id
    # 召喚
    tag @s add estT.temp
