...
今回はこれを用い、何もないvoidのワールド上で検証用functionのみを導入した状態で検証した。
...
1. **エンティティが全くいないとき**  
 [条件]  
  ...
  コマンド実行数: 4万  
  実行コマンド:  
  　　A:`execute if entity @e[tag=test]`  
  　　B:`execute if entity @e[type=armor_stand,tag=test]`  
 [結果]  
  A : **17\~19ms**  
  B : **17\~18ms**  
  あまり変わらず。  
...
2. **対象じゃないアマスタが多いとき**  
 [条件]  
  ...
  　　A:`execute if entity @e[tag=test]`  
  　　B:`execute if entity @e[type=armor_stand,tag=test]`  
  　　C:`execute if entity @e[tag=test,type=armor_stand]`  
 [結果]  
  A : **62\~63ms**  
  B : **71\~73ms**  
  C : **67\~69ms**  
